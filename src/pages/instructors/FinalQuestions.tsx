...
export default function InstructorFinalQuestions() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setError(null);
      try {
        const ref = collection(firestore, "courses", courseId, "final", "main", "questions");
  ...
  async function removeQuestion(questionId: string) {
    if (!courseId) return;
    await deleteDoc(doc(firestore, "courses", courseId, "final", "main", "questions", questionId));
    ...
    <DashboardLayout>
      <div className="p-6 space-y-6">
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <div>
            <h1 className="text-3xl font-display font-bold text-foreground">Final Exam Questions</h1>
            <p className="text-muted-foreground mt-1">Add questions for the strict final exam (3 attempts, 80% pass).</p>
          </div>
          {courseId && (
